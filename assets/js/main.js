...
(function(){
  ...
  // Show only on lander.html
  const isTargetPage = /lander\.html$/.test(path);
  if(!isTargetPage) return;

  const bd = document.createElement('div');
  bd.className = 'modal-backdrop';
  bd.innerHTML = `
    <div class="modal" style="background-color: #0f172a; color: #e5e7eb; border: 1px solid #1f2937;">
      <h3>Policy Notice</h3>
  ...
  bd.querySelector('#age-yes').addEventListener('click', function(){
    window.location.href = "https://ultrafusion.online/"; // change to your target page
  });
